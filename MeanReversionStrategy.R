...
setMethod(f="executeStrategy",signature=c("MeanReversionStrategy"),definition=function(theObject)
{
  ...
  #Scanning in macro_augmented features
  #numUnits <- scan(file = "../Macroeconomic/macro_pos.txt")
  #print(numUnits)
  #print(mode(numUnits))
  numUnits <- numUnitsLong+numUnitsShort
  cat(numUnits, file="positions", fill=FALSE)
  portfolio <- MeanReversionStrategy.getPortfolio(theObject)
  currencyPairs <- getCurrencyPairList(portfolio)
  # numUnitsPortfolio represents a matrix with the number of units of the portfolio that we buy or sell, for every timestamp
  numUnitsPortfolio <- repmat(matrix(numUnits),1,length(currencyPairs))
  #print(numUnitsPortfolio)
  ...
  zScoreDiagramString <- paste('Zscore entry ',c_entryZscore, ' std, Zscore exit ',c_exitZscore, ' std',sep="")
  ...
  plot(timeStamps,ret*100,xlab="Time",ylab="Daily Returns (%)", main=dailyReturnsString)
  
  resultString <- paste('APR: ',APR,'%, Sharpe Ratio: ',sharpeRatio,', maxDD: ',maxDD,'%',sep="")
  plot(timeStamps,results,xlab="Time",ylab="Cumulative Returns (%)",main=resultString, type='l')
